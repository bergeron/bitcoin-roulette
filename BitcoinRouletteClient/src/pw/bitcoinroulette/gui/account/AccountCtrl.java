...
public class AccountCtrl {
	...
	public void refresh(ActionEvent e) {
		...
		try {
			txs = main.player.getTransactions();
		...
		for (Transaction t : txs) {
			rows.add(new TransactionRow(t.address(), t.amount(), t.confirmations() >= 6 ? "Confirmed" : t
					.confirmations() + " confirmations"));
		}

		double balance;
		try {
			balance = main.player.getBalance();
		} catch (RemoteException e1) {
			e1.printStackTrace();
			return;
		}
	...
	public void fundAccount() {
		...
		String address;
		try {
			address = main.player.getAddress();
	...
	public void withdraw() {
		...
		final Action submitAction = new AbstractAction("Withdraw") {
			...
			public void handle(ActionEvent ae) {
				...
				String txHash = "";
				try {
					txHash = main.rouletteServer.withdraw(main.player, address, amount);
				} catch (RemoteException e) {
					e.printStackTrace();
					//TODO
				}
